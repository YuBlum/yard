...
# if DEV
  if (arena == MAP_FAILED) {
    log_errorf("%s: couldn't allocate memory for the arena", __func__);
    return 0;
  }
...
#if DEV
  if (munmap_res != 0) {
    log_errorf("%s: munmap failed", __func__);
    return false;
  }
#endif
  return true;
...
arena_is_last_alloc(struct arena *arena, void *ptr) {
  ...
  if ((uint8_t *)ptr < arena->base || (uint8_t *)ptr >= arena->base + arena->position) {
    log_errorf("%s: pointer is not valid on this arena", __func__);
    return false;
  }
#endif
...
arena_push(struct arena *arena, bool not_zeroed, size_t length) {
  ...
  if ((uintptr_t)arena->base % arena->alignment != 0) log_warnf("%s: poorly aligned arena", __func__);
#endif
  if (arena->top >= arena->capacity) {
    log_errorf("%s: arena ran out of memory", __func__);
    return 0;
  }
  length = arena_align(length, arena->alignment);
#if DEV
  if (length > arena->capacity - arena->position) {
    log_errorf("%s: arena does not have enough memory to allocate %zu bytes", __func__, length);
    return 0;
  }
#endif
...
arena_pop(struct arena *arena, size_t length) {
  ...
  if (length > arena->position) {
    log_errorf("%s: length '%zu' is greater than the arena position '%zu'", __func__, length, arena->position);
    return false;
  }
#endif
  arena->position -= length;
...
arena_scratch_begin(struct arena *arena, struct arena_state *state, size_t alignment) {
  ...
  if (!state) {
    log_errorf("%s: passing invalid state", __func__);
    return false;
  }
#endif
